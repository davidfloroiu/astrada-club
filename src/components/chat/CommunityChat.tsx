...
/**
 * Client-only wrapper for the Whop chat embed. The Whop elements runtime touches
 * browser APIs at load, so we load it with ssr: false to keep it off the server.
 */
const CommunityChatEmbed = dynamic(
  () => import("@/components/chat/CommunityChatEmbed"),
  {
    ssr: false,
    loading: () => (
      <div
      ...
      >
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted">Loading the community…</p>
        </div>
      </div>
    ),
  },
